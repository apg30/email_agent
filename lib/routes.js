...
module.exports = function (app) {
	...
	app.get('/inbox',
		...
		function handler(req, res) {
			var emails = [];
			var email_ids = [];
			models.Message.find({creator: req.user.id, mailbox: "inbox"}, function(err, messages){
				if (err) return handleError(err);

				//var inbox = db.ids_to_str(user.inbox);
				//console.log("Test subject " + user.inbox[0].subject);
				//if (err) return handleError(err);
				res.setHeader('Content-Type', 'application/json');
			...
			/*for(var i = 0; i < email_ids.length; i++) {
				console.log("Looking for " + email_ids[i]);
				models.Message.findOne({_id: email_ids[i]}, function(err, email){
					if (err) return handleError(err);
					else {
						console.log("Found " + email_ids[i] );
						emails.push(email);
					}
