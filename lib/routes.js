...
*******************************************************************************/
	...
	app.get('/pop3_retr',
		...
		function handler(req, res) {
			//Check we have enough params given to continue
			if(typeof req.query.number == "undefined" && typeof req.query.pop3_id == "undefined"){
				console.log("not enough args");
				res.sendStatus(500);
				return;
			} else if (req.query.number  && req.query.pop3_id){
			...
			models.Settings
				...
				.exec(function(error, settings) {
					...
					//Set max amount to list
					if(typeof req.query.number != "undefined"){
						pop3c.set_list_max(req.query.number);
					}
	...
	*/
	app.get('/pop3_dele',
		...
		function handler(req, res) {
			...
			pop3c.connect(config.pop3_host, config.pop3_port);

			//Logs in user to POP3 server
			pop3c.login(config.pop3_username, config.pop3_password);
	...
	app.get('/pop3_list',
		...
		function handler(req, res) {
			...
			if(typeof req.query.number != 'undefined'){
				...
				pop3c.set_max_list(req.query.number);
			}
