...
module.exports = function(app) {
    ...
    function sent_insert_db(user_id, request, res) {
        ...
        var message = new models.Message({
            ...
            cc_emails: cc_list,
            bcc_emails: bcc_list,
            from_emails: request[name + "_from"],
            subject: request[name + '_subject'],
            content: "Test content",
            creator: user_id,
            html: request[name + '_content'],
    ...
    app.post('/smtp_send',
        function handler(req, res) {
            ...
            sent_insert_db(req.user.id, post_params, res);
    ...
    app.post('/smtp_reply',
        function handler(req, res) {
            ...
            smtp.send_email(post_params, res);
            sent_insert_db(req.user.id, post_params, res);
            req.flash("info", "Replied");
            res.redirect('/');
        }
    );

    app.post('/smtp_reply_all',
        function handler(req, res) {
            //Ensure client is on HW network.
            var post_params = req.body;
            smtp.send_email(post_params, res);
            sent_insert_db(req.user.id, post_params, res);
            req.flash("info", "Replied To All");
            res.redirect('/');
        }
    );

    app.post('/smtp_forward',
        function handler(req, res) {
            //Ensure client is on HW network.
            var post_params = req.body;
            smtp.send_email(post_params, res);
            sent_insert_db(req.user.id, post_params, res);
            req.flash("info", "Forwarded Email");
            res.redirect('/');
        }
    );
    ...
    app.get('/db_inbox',
        ...
        function handler(req, res) {
            models.Message.find({
                creator: req.user.id,
                mailbox: "inbox"
            }, "to_emails cc_emails bcc_emails subject", function(err, messages) {
                if (err) return handleError(err);
                res.setHeader('Content-Type', 'application/json');
    ...
    app.post('/update_background',
        function(req, res) {
            models.Settings.findOneAndUpdate({
                ...
                background_change: req.body.update_background
            }, {
                upsert: true
            }, function(err, doc) {
                if (err) return res.send(500, {
