...
*******************************************************************************/
	...
	app.get('/validate',
		function(req, res){
			...
			req.end();*/
			res.render("index");
  		//res.sendFile(config.build_dest + "index.ejs");
	...
	app.post('/update_background',
		function(req, res){
			models.Settings.findOneAndUpdate({_id:req.user.settings}, {background_change:req.body.background_change}, {upsert:true}, function(err, doc){
			    if (err) return res.send(500, { error: err });
				//req.flash('info', 'Settings saved successfully');
			    res.status(200);
		});
