...
var shared_data = {
	...
	list: {body: "", array: [], max: -1, idx: 0, mode: list_modes.ALL}, //-1 undicates list all, idx used to iterate until max
	retr: {body: "", id: 0},
	user_id: 0,
	...
	if(data){
		...
		if(cmd == "+OK") {
			...
			if(current_state == states.IDLE){
				...
				shared_data.list.idx = 0;

				//Work out how many list items to list.
				if(shared_data.list.mode == list_modes.ALL) {
					shared_data.list.max = list_return.length;
				}

				console.log("getting" + shared_data.list.max + "messages");

				var temp_data;
				for(var i = 0; (i < list_return.length)  && (shared_data.list.idx < shared_data.list.max); i++){
			...
			if (current_state == states.RETR_WAITING){
				...
				for(var i = 0; (i < list_return.length)  && (shared_data.list.idx < shared_data.list.max); i++){
					console.log("idx: " + shared_data.list.idx + "max: " + shared_data.list.max);
					temp_data = list_return[i].split(" ");
					if(temp_data[0] != "+OK" && temp_data[0] != "" && temp_data[0] != "."){
						shared_data.list.array.push({id : parseInt(temp_data[0]), bytes: parseInt(temp_data[1])});
						shared_data.list.idx++;
...
POP3Client.prototype.connect = function (host, port){
	//On connect, set all gloabls to their default values
	//-1 Indicates fetch all
	shared_data.list.max = -1;
...
POP3Client.prototype.set_max_list = function (number){
	if(number > -1){
		shared_data.list.mode = list_modes.NUMBER;
		console.log("setting max to " + number);
	}else{
		shared_data.list.mode = list_modes.ALL;
	}
	shared_data.list.max = number;
}
